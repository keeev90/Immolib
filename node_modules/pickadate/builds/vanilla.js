...
(function (global, factory) {
  ...
  var getTranslatedPartialState = function getTranslatedPartialState(partialState, partialTranslation) {
    ...
    return _objectSpread({}, partialState, {
      firstDayOfWeek: partialTranslation.firstDayOfWeek != null ? partialTranslation.firstDayOfWeek : partialState.firstDayOfWeek,
      template: partialTranslation.template != null ? partialTranslation.template : partialState.template,
      templateHookWords: _objectSpread({}, partialTranslation.templateHookWords, partialState.templateHookWords)
    });
  };

  var getInitialState = function getInitialState(partialState) {
    var view = getStartDateOfMonth(new Date());
    return mergeUpdates({
      firstDayOfWeek: 0,
      selected: null,
    ...
    }, copyDefinedValues(partialState));
  };
  ...
  var createGetValue = function createGetValue(store) {
    return function () {
      ...
      return selected ? format(selected, template, templateHookWords) : '';
    };
  };
  ...
  function createPicker() {
    ...
    var initialState = getInitialState(getTranslatedPartialState(partialState, partialTranslation));
    var store = createStore(initialState);
    var getValue = createGetValue(store);
    var currentValue = getValue();

    var subscribeToValue = function subscribeToValue(listener) {
  ...
  var renderLayout = createRootBox(createHeaderBox(createMonthAndYearBox(), createPreviousButton(), createTodayButton(), createNextButton()), createBodyBox(createGridButton()), createFooterBox(createTimeBox(createHourInput(), createTimeSeparator(), createMinuteInput(), createMeridiemButton()) // renderer.createClearButton()
  ));
  ...
  var render = function render(element, picker, options) {
    ...
    var containerElement = renderLayout(picker, options, element);
